 ...
 */
exports.sendMessageToMentionUsers = function (text, topic_id, sender_id, reply_id, callback) {
  ...
  userManager.getUsersByLoginids(fetchUsers(text), function (err, users) {
    if (err || !users) {
      return callback(err);
    }
    ...
    users.forEach(function (user) {
      Message.sendAtMessage(user._id, sender_id, topic_id, reply_id, ep.done('sent'));
    });
  });
};
